...
async function querySearXNG(
    ...
    if (PUBLIC_FALLBACK_INSTANCES.length > 0) {
        console.log(`[SearXNG] Testing ${PUBLIC_FALLBACK_INSTANCES.length} public instances...`);
        const racePromises = PUBLIC_FALLBACK_INSTANCES.map(async (instance) => {
            try {
                ...
                console.warn(`[SearXNG] Public instance failed: ${instance} - ${e instanceof Error ? e.message : String(e)}`);
                throw e;
            }
        });
        ...
        try {
            ...
            console.log(`[SearXNG] Public race succeeded.`);
            return result;
        } catch (err: unknown) {
            console.error(`[SearXNG] ALL public instances failed. Error:`, err);
            return [];
        }
    }
...
async function searchSearXNG(query: string, queryInfo: QueryClassification, isFollowUp: boolean = false): Promise<SearchResult[]> {
    ...
    // If it's a follow-up query generated by the LLM, it's already highly specific.
    // We don't need to add 3-4 more variations of it, which would DDoS our own SearXNG instance.
    if (isFollowUp) {
        const results = await searchPromises[0]().catch(() => [] as SearchResult[]);
        return results;
    }
    ...
    // Strategy 5: For technical queries
